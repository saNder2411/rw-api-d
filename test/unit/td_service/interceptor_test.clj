(ns unit.td-service.interceptor-test
...
(deftest interceptor-test
  ...
  (testing "entity-render-test"
    (let [ctx {:result {:body {:title "A-List" :items {}}}}]
      (is (=
            ...
            (:response (chain/execute ctx [(i/interceptor r/entity-render)])))))
  ...
  (testing "lists-view"
    (let [ctx {:request {:database {}}}]
      (is (=
            {:body {}}
            (:result (chain/execute ctx [(i/interceptor r/lists-view)]))))))

  (testing "list-view"
    (let [ctx {:request {:path-params {:list-id 1} :database {1 {:title "A-List" :items {}}}}}]
      (is (=
            {:body {:title "A-List" :items {}}}
            (:result (chain/execute ctx [(i/interceptor r/list-view)])))))))
